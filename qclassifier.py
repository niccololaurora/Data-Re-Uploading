...
class SingleQ_classifier():
    ...
    def __init__(self, layers=1, grid=None, training_sample=200, test_samples=4000, method="l-bfgs-b", seed=0, foldername=None):
        self.epochs = 100
        self.layers = layers
        self.learning_rate = 0.001
        self.params = np.random.randn(4 * self.layers)
        self.targets = [np.array([1, 0], dtype="complex"), np.array([0, 1], dtype="complex")]
        # self.training_data = create_dataset(grid = grid)
        self.training_data = create_dataset(samples = training_sample, seed=seed)
    ...
    def cost_function_fidelity(self, parameters=None):
        ...
        '''
        Queste tre stringhe servono per calcolare la loss function scegliendo i pesi.
    ...
    def training_loop(self, record=True):
        ...
        if self.method == "Adamax":
            ...
            def opt_step():
                ...
                if record is True:
                    filename = "Gradients_" + method + ".txt"
            ...
            m = minimize(lambda p: loss(p), self.params, method="l-bfgs-b", options={"disp": True}, tol=1e-10)
            best_loss = m.fun
            best_parameters = m.x
    ...
    def paint_guesses(self, guess_labels, name=None):
        ...
        plt.scatter(x_0, x_1, s=6, c=checks, cmap=colors_rightwrong, norm=norm_rightwrong)

        plt.xlabel("x")
        plt.ylabel("y")
        plt.title(f"Layers: {self.layers}, Method: {self.method}, Data (T, V): {self.ntrainingsample, self.ntestsample}")
